 ...
 * Col D formula: =IF(OR(B="Angel's Pizza",B="Figaro"),IF(C="","⚠ MISSING REGION","OK"),"N/A")
...
function rebuildSettingsHeaders() {
  ...
  // =IF(OR(B2="Angel's Pizza",B2="Figaro"),IF(C2="","⚠ MISSING REGION","OK"),"N/A")
  if (lastRow >= 2) {
    const dFormulas = [];
    for (let row = 2; row <= lastRow; row++) {
      dFormulas.push([
        `=IF(OR(B${row}="Angel's Pizza",B${row}="Figaro"),IF(C${row}="","⚠ MISSING REGION","OK"),"N/A")`,
      ]);
    }
