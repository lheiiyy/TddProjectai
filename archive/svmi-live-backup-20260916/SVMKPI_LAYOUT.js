...
function buildTitle(sheet) {
  _merge(sheet, 'C2:I2');
  sheet.getRange('C2')
    .setValue('STORE VISIT PROGRAM — EXECUTIVE SUMMARY 2026')
...
function buildMonthly(sheet) {
  ...
  const monthHeaders = ["MONTH", "ANGEL'S PIZZA", "APEX", "FIGARO", "TIEN MA'S", "KOOBIDEH", "TOTAL"];
...
function buildBrandPerformance(sheet) {
  ...
  const brands = ["ANGEL'S PIZZA", 'APEX', 'FIGARO', "TIEN MA'S", 'KOOBIDEH'];
...
function _buildESFormulas(sheet) {
  ...
  const brands = ["ANGEL'S PIZZA", 'APEX', 'FIGARO', "TIEN MA'S", 'KOOBIDEH'];
  const monthEnds = [31,28,31,30,31,30,31,31,30,31,30,31]; // 2026
  ...
  for (let m = 0; m < 12; m++) {
    ...
    brands.forEach(brand => {
      rowFormulas.push(
        `=COUNTIFS(${ML}!D:D,"${brand}",${ML}!B:B,">="&DATE(2026,${month},1),${ML}!B:B,"<="&DATE(2026,${month},${monthEnds[m]}))`
      );
    });
  ...
  const dateFrom = Array.from({length:12},(_,i)=>`DATE(2026,${i+1},1)`).join(',');
  const dateTo   = Array.from({length:12},(_,i)=>i<11?`DATE(2026,${i+2},1)`:'DATE(2027,1,1)').join(',');
