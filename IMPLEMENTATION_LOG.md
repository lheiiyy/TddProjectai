...
| 2026-09-26 | (this session) | Migrate Legacy Data bug fix | **Fixed a silent data-loss bug in `settingsMigration_run()` a live-deployment admin reported**: "Migrate Legacy Data" reported success while some SETTINGS rows never became CONFIG_* entities, with no error shown. Root cause: `store_migrateFromSettings()` (`SVMKPI_STORE_CONFIG.gs`), `visitor_migrateFromSettings()` (`SVMKPI_VISITOR_CONFIG.gs`), and `purpose_migrateFromSettings()` (`SVMKPI_PURPOSE_CONFIG.gs`) each called `store_create()`/`cfg_createConfiguration()`/`purpose_create()` and only handled the success case (`if (result.success) created.push(...)`) — any rejection (most commonly a blank required field: Brand/Region/Category for a Store) was silently dropped, with no `else` branch at all. All three now collect a `failed: [{name, message}]` array and return it; `SVMI_PORTAL.html`'s `runSettingsMigration()` surfaces each failure as a warning log line with its reason, and marks the tool-stat "Done with errors" instead of a plain success when anything failed. (Also documented, not fixed, a related but separate gap found while investigating: `cfg_validateConfiguration()` for `CFG_AREA.STORES` checks Brand/Region/Category for blankness only, not membership in `APPROVED_BRANDS`/`APPROVED_REGIONS`/`APPROVED_CATEGORIES` — a non-blank-but-unapproved value is currently accepted.) 11 new tests (`settings-config-migration.test.js` grew from 34 to 45 assertions, proving a row with a blank required field is now reported in `failed` with the correct reason, not silently dropped) plus the full existing suite, all passing — 1284 assertions across 26 files. |
| 2026-09-26 | (this session) | Visited This Month vs. Executive Summary reconciliation fix | **Fixed a live undercount a project owner reported**: Store Insights → "Visited This Month" (September, live) totaled 88 visits while the live Executive Summary's Monthly-by-Brand table totaled 106 for the same month — an 18-visit gap. Root cause, confirmed by direct comparison: Executive Summary's `COUNTIFS` (`_buildESFormulas()`, `SVMKPI_LAYOUT.gs`) counts every `MASTER_LOG` row matching Brand+Date with no roster check at all, while `sl_getVisitedThisMonth()` (`SVMKPI_STORE_LOOKUP.gs`) silently skipped any row whose Store text didn't resolve to a *currently active* SETTINGS entry (renamed, deactivated/removed, or a text mismatch) — the same "silently drop instead of reporting" shape as the Migrate Legacy Data bug above, in a different function. Fix: `sl_getVisitedThisMonth()` now returns `{ resolved, unmapped }` instead of a flat array — `unmapped` surfaces exactly the rows that don't resolve to a current roster entry (raw store text, visit count, last visit date), reusing the existing `CONFIG_UNMAPPED_STORES` precedent (`SVMKPI_STORE_CONFIG.gs`, Phase 1B) instead of inventing a new pattern; `resolved` visits + `unmapped` visits now reconciles with Executive Summary's raw count. Executive Summary itself was deliberately left unchanged (roster-scoping it would retroactively change historical monthly totals whenever a store is later renamed/removed — a bigger, riskier change than what was actually broken). Also added optional `monthNumber`/`reportingYear` params (mirroring `sl_getComplianceGaps()`'s existing fallback pattern) so this tab isn't permanently locked to "whatever month it is right now." `SVMI_PORTAL.html`'s Store Insights tab gained a month selector (mirroring the Unvisited-This-Month tab's `#monthSel`/`#monthLbl` pattern) and an "⚠ N visits not matched to a current store" warning block. 5 new tests (`store-lookup-date-handling.test.js` grew from 22 to 27 assertions, proving unmapped rows are surfaced not dropped, that resolved+unmapped visits reconciles with a raw count, that brandFilter exclusion is never mistaken for "unmapped," and that an explicit past month is now reachable) plus the full existing suite, all passing — 1289 assertions across 26 files. |

**The application did not change between Phase 1G and Phase 1H-B.1.** The
System Peripherals audit, Phase 1H-A, and Phase 1H-B (three of the ten
rows above) were documentation/audit/architecture-only work — no
`.gs`/`.html` file was touched by any of them. **Phase 1H-B.1, Phase
1H-C, Phase 1H-C Security Fix R1, Phase 1H-C Security Fix R2, the
Migrate Legacy Data bug fix, and the Visited This Month reconciliation
fix are the six code changes since Phase 1G** — see those rows above and
`reviews/005-...md`/
